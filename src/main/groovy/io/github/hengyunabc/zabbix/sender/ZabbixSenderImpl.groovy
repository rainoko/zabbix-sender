...
public class ZabbixSenderImpl implements ZabbixSender {
	def String host;
	def int port;
	def int connectionTimeoutMillis = 3000;
	def int socketTimeoutMillis = 3000;

	def ZabbixSenderImpl(String host, int port) {
		this.host = host;
		this.port = port;
	}

	def ZabbixSenderImpl(String host, int port, int connectionTimeoutMillis,
